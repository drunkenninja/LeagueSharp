...
namespace NiceGuySharp
{
    public class Program
    {
        ...
        public static int MinTimeBeforeNewMessage = Rand.Next(5000, 9000);
        ...
        public static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Menu = new Menu("Nice Guy Sharp", "niceguy", true).SetFontStyle(System.Drawing.FontStyle.Regular ,Color.Green);
            ...
            Game.PrintChat("Nice Guy Sharp // Reloaded // Loaded!", Color.Green);
            Game.PrintChat("Original by imsosharp - Edited by Art3mis", Color.Green);
            Game.PrintChat("NOTE: If nothing is said, remove the text files associated with this script and reload. The file reading code was changed.", Color.Red);
            ...
            try
            {
                if (Menu.Item("enabled").GetValue<bool>())
                {
                    if (Environment.TickCount - LastSentMessage > MinTimeBeforeNewMessage)
                    {
                        if (message != null)
                        {
                            Utility.DelayAction.Add(message.Length * (new Random(Environment.TickCount).Next(100, 200)), () => Game.Say(message));
                        }
                    }
                }
            }
